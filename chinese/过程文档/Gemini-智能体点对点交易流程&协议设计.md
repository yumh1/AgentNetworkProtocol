...
```stateDiagram-v2
    [*] --> New
    New --> Pending_Payment: User Confirms (VC w/ Offer Hash)
    Pending_Payment --> Escrowed: Funds Escrowed (VC w/ Payment Req/Instruction Hash)
    Pending_Payment --> Cancelled: Payment Timeout / User Cancel
    Escrowed --> In_Service: Hotel Confirms Booking (Optional VC w/ Escrowed Hash)
    Escrowed --> Cancelled: User Cancel (Policy Permitting)
    Escrowed --> In_Dispute: Dispute Initiated (VC w/ Escrowed Hash)
    In_Service --> Pending_Settlement: User Confirms Service (VC w/ Escrowed/Hotel Confirm Hash) / Auto Confirm Timeout
    In_Service --> In_Dispute: Dispute Initiated (VC w/ In Service state Hash)
    Pending_Settlement --> Settled: Funds Released (VC w/ Service Confirm Hash)
    Pending_Settlement --> In_Dispute: Dispute Initiated (VC w/ Pending Settlement Hash)
...
**Table 2: Core State Transitions (with VC Hash Chain emphasis)**

| 当前状态 | 触发事件/凭证类型 | 主要动作/验证 | 下一状态 | 备注 |
|---------|-----------------|--------------|--------|------|
| New / Offered | BookingConfirmationCredential (User) | 验证用户签名，提议引用，验证credentialSubject中的哈希值与OrderOfferCredential匹配 | Pending Payment | |
| Pending Payment | FundsEscrowedCredential (Payment) | 验证支付平台签名，托管金额，验证credentialSubject中的哈希值与PaymentInstructionCredential匹配 | Escrowed | |
| Pending Payment | 支付超时 | 支付智能体检测超时，发送通知 | Cancelled | 需要定义超时时长 |
| Escrowed | (内部逻辑/可选BookingFinalizedCredential) | 酒店确认预订资源，（若有VC）验证credentialSubject中的哈希值与FundsEscrowedCredential匹配 | In Service | 假设服务按期开始 |
| In Service | ServiceCompletionConfirmationCredential (User) | 验证用户签名，确认服务完成，验证credentialSubject中的哈希值与FundsEscrowedCredential或BookingFinalizedCredential匹配 | Pending Settlement | |
| In Service | 用户确认超时 | 支付智能体根据预设规则触发 | Pending Settlement | 需要明确定义自动确认规则和时长 |
| Pending Settlement | (内部资金划转完成) | 支付智能体执行结算，生成FundsReleasedCredential（其credentialSubject包含ServiceCompletionConfirmationCredential的哈希） | Settled | |
| Escrowed / In Service / Pending Settlement | DisputeInitiationCredential (User/Hotel) | 验证签名，暂停资金操作，验证credentialSubject中的哈希值与当前状态最后一个VC匹配 | In Dispute | |
| In Dispute | DisputeResolutionCredential (Arbitrator) | 验证仲裁者签名，根据裁决内容执行资金操作，（若有VC）验证credentialSubject中的哈希值与DisputeInitiationCredential匹配 | Settled / Cancelled | 仲裁结果驱动最终状态 |
| 任意状态 | CancellationRequestCredential (User) | 根据当前状态和取消策略处理，（若有VC）验证credentialSubject中的哈希链接 | Cancelled / (不变) | 取消策略可能复杂 |
