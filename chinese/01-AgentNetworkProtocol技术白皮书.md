...
在 DID 文档中，我们引入了专门的验证方法`humanAuthorization`，用于区分人类授权与智能体自动授权。对于低风险、常规性的请求（如查询公开信息），用户智能体可以自动代表用户进行授权；而对于高风险、涉及隐私或财产的重要请求（如支付交易），则必须经过人类明确授权。用户智能体在发起重要请求时，需使用`humanAuthorization`方法进行签名，这要求智能体先向人类用户发起授权请求，获得确认后再执行操作。智能体开发者需安全保管相关私钥，并实施严格的权限隔离措施，如通过生物识别验证用户身份后才能使用该私钥进行签名。

为保护用户隐私，我们建议采用多DID策略。这包括为用户生成一个相对稳定的主DID用于长期社交关系，同时生成多个子DID用于不同应用场景（如购物、订餐等）。每个DID具有不同的角色和权限，使用不同的密钥对，实现细粒度的权限控制。此外，定期停用过期的子DID并申请新的DID，可有效提高隐私安全防护水平，防止用户被跨平台追踪和分析。智能体在通信过程中应遵循最小信息披露原则，只传输完成任务所必需的信息。
...
为降低通信成本并提升交互体验，在大多数通信场景中，智能体应避免每次都通过元协议进行通信协议的协商。因此，在应用协议层，我们将基于语义网相关标准设计一系列规范，包括**智能体能力与支持协议描述**和**应用协议管理规范**，使得智能体之间的通信更加便捷、高效和低成本。

**智能体能力与支持协议描述规范**明确了智能体如何描述自身的能力和支持的协议，以及调用这些能力所需的协议信息。智能体可以将这些描述文档发布在互联网或专门的智能体搜索服务上，供其他智能体查询和调用。

在智能体描述规范设计上，我们使用了很多语义网的标准，包括RDF（Resource Description Framework），JSON-LD(JSON Linked Data)，schema.org等。重复的利用这些技术，可以提升两个智能体之间对数据理解的一致性。
